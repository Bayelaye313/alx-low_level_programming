...
/**
 * verify_elf_magic - Verifies if a file is an ELF file.
 * @e_ident: A pointer to an array containing the ELF magic numbers.
 *
 * Description: If the file is not an ELF file, it exits with code 98.
 */
void verify_elf_magic(unsigned char *e_ident)
{
	int i;
...
/**
 * print_os_abi_version - Prints the ABI version of an ELF header.
 * @e_ident: A pointer to an array containing the ELF ABI version.
 */
void print_os_abi_version(unsigned char *e_ident)
{
...
{
	Elf64_Ehdr *header;
	int file_descriptor, read_bytes;
	...
	verify_elf_magic(header->e_ident);
	...
	print_os_abi_version(header->e_ident);
